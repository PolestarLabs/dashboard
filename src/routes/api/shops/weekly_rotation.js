...
module.exports = async (query,user,rarArray) => {
	...
	let allBGs = shuffle( await DB.cosmetics.find(filterQuery, {_id:0}), SEED);

	const start = Date.now();
	const previous_payload3 = rotationSetsCache.get(preseed3) || getFinalRotation(allBGs,rarArray, preseed3);
	const previous_payload2 = rotationSetsCache.get(preseed2) || getFinalRotation(allBGs,rarArray, preseed2, previous_payload3);
	const previous_payload = rotationSetsCache.get(PREV_SEED) || getFinalRotation(allBGs,rarArray, PREV_SEED, previous_payload2);

	const payload = rotationSetsCache.get(SEED) || getFinalRotation(allBGs,rarArray, SEED, previous_payload);
	const end = Date.now();
	...
	rotationSetsCache.set(yearMonthFifteenWeek,payload);
		...
		if (burnInfo) {
			let xxxx= await DB.users.set(user, {
				$set: {"switches.custom_rt": { seed: SEED, items: payload.map(x=>x.code) } }
			});
		}
...
function getFinalRotation(allBGs,rarArray, SEED, previous ) {
