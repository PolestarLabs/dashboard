...
STICKERS = new Vue({
  ...
  methods:{
      packProgress(){
        ...
        let val = userdata.modules.stickerInventory.filter(x=> thisPack.includes(x) ).length
        let max = thisPack.length;
        
        return {pct: Math.floor(100/(val==0?100:(max / val))), val,max}
    },
    hasSticker(stk){
        return this.toggleOwned? userdata.modules.stickerInventory.includes(stk.id):true;
    },
    calculateTime(item){
