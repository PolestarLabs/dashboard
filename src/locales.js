...
function loadLangJSONFiles (langPath, defaultLang,siteLangs) {
  ...
  if (files) {
    for (var i = 0; i < files.length; i++) {
      if ((files[i].split('.').pop() === 'json' && files[i].substr(0, 1) !== '.' && files[i].split('.').shift())||true) {
       // if (!siteLangs.includes((files[i]))) return;
        try {
...
exports = module.exports = function (opts) {
  ...
  i18nTranslations = loadLangJSONFiles(translationsPath, defaultLang,siteLangs);
  ...
  return function i18n (req, res, next) {
    ...
    while (1) {
      if (cookieLangName && alreadyTryCookie === false) {
        ...
        if (wLang.length) {
          if (!siteLangs.includes(wLang[0]))computedLang = defaultLang;
          else computedLang = wLang[0];
    ...
    if (computedLang in i18nTranslations) {
      ...
      if (computedLang.indexOf('-') > -1) {
        ...
        if (soloLang in i18nTranslations) {
          //console.log(textsVarName)
          //console.log( i18nTranslations[soloLang])
