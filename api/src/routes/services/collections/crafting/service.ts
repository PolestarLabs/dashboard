/**
 * crafting/service.ts
 * Pure business logic — no Elysia Context imported here.
 * Accepts typed payloads, returns plain objects.
 */

import { RARITY_VALUES, RarityType as Rarity } from "@definitions/Rarity";
import { shuffle, isExact } from "@routes/models/collections";
import type { PotItem, MixBody, MixResponse, CreateBody, CreateResponse } from "./model";
...
export class CraftingService {
    ...
    /**
     * Discover what can be crafted from the given pot of materials.
     * Falls back to type-based matching when no exact recipe exists.
     */
    static async mix(payload: MixBody, userId: string | undefined, DB: any): Promise<MixResponse> {
        ...
        if (!isBroadMatch) {
                    ...
                    .map((itm) => {
                        const threshold =
                            refinedPot
                                ...
                                .reduce<{ count: number }>((a, b) => ({ count: a.count + b.count }), { count: 0 }).count ?? 0;
            ...
            if (isSameTypeMatch) {
                ...
                possible = possible.filter((x: any) => {
                    ...
                    return ri >= (RARITY_VALUES.indexOf(lowestRar) || 1) && ri <= RARITY_VALUES.indexOf(highestRar);
                });
            }
    ...
    /**
     * Execute a craft: consume materials and add the crafted item to inventory.
     * Returns `{ ok, code, body }` so the controller can call status(code, body).
     */
    static async craft(
        payload: CreateBody,
        userId: string,
        ...
        if (!itemToCraft?.crafted)
            return { ok: false, code: 403, body: { status: "ERROR", message: "This item can't be crafted" } };

        const userData = await DB.users.getFull(userId);
        if (!userData)
            return { ok: false, code: 401, body: { status: "ERROR", message: "Not Logged in" } };
        ...
        return {
            ok:   true,
            code: 200,
            body: { status: "OK", message: "Item has been crafted", inventory: userData.modules.inventory },
        };
    }
}
