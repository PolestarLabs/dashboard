...
function rarityCompare(a: InventoryItem, b: InventoryItem): number {
  const ia = RARITY_VALUES.indexOf(a.rarity);
  const ib = RARITY_VALUES.indexOf(b.rarity);
  return ia - ib;
}
...
export class CraftingService {
  ...
  static async mix(
    ...
    const getIdentifiedItem = (item: PotItem|InventoryItem): InventoryItem => {
      const returnItem = identifiedPotItems.find((i) => i.id === item.id);
      if (!returnItem) throw new Error(`Item not found: ${item.id}`);
      return returnItem;
    };

    const potTypeMap = identifiedPotItems.map((i) => i.type);
    ...
    if (!isBroadMatch) {
      ...
      const refinedPot: PotItem[] = pot.map((item) => ({
        ...item,
        count: item.count * ((RARITY_VALUES.indexOf(
          getIdentifiedItem(item)!.rarity) + 1) / 2),
      }));

      const querySameType = {
        $and: identifiedPotItems
          .map((itm) => {
            const threshold =
              refinedPot
                .filter((i) => getIdentifiedItem(i).type === itm.type)
                .reduce<{ count: number }>(
        ...
        possible = [
          possible.sort((a, b) => {
            ...
            const aFits = (a.materials).every(
              (m) => m.count <= (findInPot(m.id, pot)?.count ?? 0),
            );
            return aFits ? -1 : 0;
          })[0]!,
        ];
